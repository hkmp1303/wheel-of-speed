...
function useMessageProcessor(matchState, playerId, activePlayerName) {
  ...
  useEffect(() => {
    ...
    const isRoundStartMessage =
      matchState?.lastMessage?.startsWith("Round ") &&
      matchState?.lastMessage?.includes("started");
    ...
    if (isRoundStartMessage) {
      ...
      message =
        "Spin the wheel to lock in your reward and reveal the first letter.";
      messageClass = null;
    }
...
export default function MatchPage() {
  ...
  const { displayMessage, displayMessageClass } = useMessageProcessor(
    matchState,
    playerId,
    activePlayerName,
  );
  ...
  async function handleSpinButton() {
    ...
    if (connection.state !== "Connected") {
      console.error(
        "Cannot spin: SignalR not connected (state=",
        connection.state,
        ")",
      );
      return;
    ...
    try {
      ...
      handleSpinComplete(
        result?.currentWheelValue ?? result?.rewardAmount ?? null,
      );
    } catch (err) {
    ...
    <main id="game-board" className="page wide">
      <section className="card hero">
        ...
        >
          <p>
            Current player:{" "}
            <strong>{matchState.activePlayerName ?? "Waiting..."}</strong>
          </p>
          <div
            className="timer"
          ...
          >
            <p>
              Timer: <strong>{matchState.secondsLeft}</strong>
            </p>
          </div>
        </div>
        ...
        <div className="actions">
          <button
            ...
            disabled={
              !myTurn ||
              (!isInProgress && !isRoundEnded) ||
              (isInProgress && hasSpun) ||
              isFinalGuess
            }
          >
            Spin
        ...
        <div className="messages">
          {displayMessage && (
            <p>
              <em className={displayMessageClass}>{displayMessage}</em>
            </p>
          )}
        </div>
        ...
        <form onSubmit={submitGuess} className="guess-form">
          ...
          <button
            type="submit"
            disabled={!myTurn || !guessText.trim() || !hasSpun || guessDisabled}
          >
            Guess
          </button>
        </form>
        ...
        {matchState.status === "Finished" && (
          <div className="ending-box">
            ...
            {rematchState &&
              rematchState.status === "challenged" &&
              rematchState.responderPlayerId === playerId && (
                <div
                  style={{
                    marginTop: "1rem",
                    padding: "1rem",
            ...
            {rematchState &&
              rematchState.status === "challenged" &&
              rematchState.challengerPlayerId === playerId && (
                <p
                  style={{
                    marginTop: "1rem",
                    fontStyle: "italic",
